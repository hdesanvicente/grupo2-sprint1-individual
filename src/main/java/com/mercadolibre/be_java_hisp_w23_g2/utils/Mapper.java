...
public class Mapper {
    ...
    public static PostBasicDTO mapPostToPostBasicDTO(Post post) {
        return new PostBasicDTO(post.getUserId(),post.getId(), post.getDate(),mapProductToProductDTO(post.getProduct()),post.getCategory(),post.getPrice());
    }
    ...
    public static ProductBasicDTO mapProductToProductDTO(Product product) {
        return new ProductBasicDTO(product.getId(), product.getName(), product.getType(), product.getBrand(), product.getColor(), product.getNotes());
    }

    public static Product mapProductDTOToProduct(ProductBasicDTO productBasicDTO) {
        return new Product(productBasicDTO.getId(), productBasicDTO.getName(), productBasicDTO.getType(), productBasicDTO.getBrand(), productBasicDTO.getColor(), productBasicDTO.getNotes());
    }
